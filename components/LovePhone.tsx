...
const LovePhone: React.FC = () => {
  ...
  const handlePickUp = async () => {
    if (isAudioLoading || isPlaying) return;
    ...
    try {
      const audioBase64 = await speakMessage(FIXED_MESSAGE);
      if (audioBase64) {
        await playAudio(audioBase64);
      }
    } catch (error) {
      console.error(error);
      // 若語音失敗，文字已顯示，確保使用者仍能閱讀
    } finally {
      ...
      <div className="mt-8 text-center relative z-10 w-full">
        ...
        <div className="mt-4 flex flex-col items-center gap-1">
          {isAudioLoading && !isPlaying && (
            <span className="text-[10px] text-red-400 font-bold animate-pulse">語音生成中，請稍候...</span>
          )}
